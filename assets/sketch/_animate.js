...
const handleClick = (e) => {
  if (!isDragging) {
	...
	if (!(
		...
		if (target != 'Scene') {
			console.log(`${target}: (${pos.x}, ${pos.y}, ${pos.z}, ${tar.x}, ${tar.y}, ${tar.z})`);
			control.setLookAt(..._Coordinates[target], true);
		}
		// select mesh
		else {
			console.log(`${intersects[0].object.name}: (${pos.x}, ${pos.y}, ${pos.z}, ${tar.x}, ${tar.y}, ${tar.z})`);
			control.setLookAt(..._Coordinates[target], true);
		}
	}
  }
